...
class FAMDock():
    ...
    def make_receptor(self,receptor_path):
        if receptor_path[-4:] != ".pdb":
            ...
            return
        self.receptor = Receptor(receptor_path,"workdir_fam/qdock_output/receptor.pdbqt")
    ...
    def make_ligand(self,ligand_paths):
        if self.__step != "ligand":
            ...
            return
        self.ligands = []
        if not os.path.exists("workdir_fam/qdock_output/Ligands"):
            os.mkdir("workdir_fam/qdock_output/Ligands")
        for path in ligand_paths:
            self.ligands.append(Ligand(path,"workdir_fam/qdock_output/Ligands"))
    ...
    def make_box_ligand(self,path,
        ...
        if self.__step != "box":
            print("Please run make_%s first!"%(self.__step))
            return
        if not os.path.exists("workdir_fam/qdock_output/Box_Rawligand"):
            os.mkdir("workdir_fam/qdock_output/Box_Rawligand")
        self.raw_ligand = Ligand(path,"workdir_fam/qdock_output/Box_Rawligand")
    ...
    def indiv_dock(self,ligand,edge_cutoff,K_dist,K_mono,n_pos=30,
        ...
        if not os.path.exists("workdir_fam/qdock_output/QUBOs"):
            os.mkdir("workdir_fam/qdock_output/QUBOs")
        if save_qubo:
            np.save("workdir_fam/qdock_output/QUBOs/%s.npy"%(ligand.name),qubo)
        ...
        if not os.path.exists("workdir_fam/qdock_output/Matches"):
            os.mkdir("workdir_fam/qdock_output/Matches")
        if save_match and match:
            np.save("workdir_fam/qdock_output/Matches/%s_match.npy"%(ligand.name),np.array(match, dtype=object), allow_pickle=True) 
        if not os.path.exists("workdir_fam/qdock_output/Poses"):
                os.mkdir("workdir_fam/qdock_output/Poses")
        if save_pose and news:
            tmp = ligand.ligand.copy()
            tmp.setCoords(news[0])
            if len(news) > 1:
                tmp.addCoordset(np.array(news[1:]))
            prody.writePDB("workdir_fam/qdock_output/Poses/%s_poses.pdb"%(ligand.name),tmp)
        return np.array(news)
    
    
    def __autosite(self):
        self.dims = (self.box_lengths / self.grid_length).astype(int)
        for i in range(3):
            if self.dims[i] % 2 == 0:
                self.dims[i] = self.dims[i] - 1
        os.system("mkdir workdir_fam/qdock_output/pocs")
        os.system("autosite -r %s --boxcenter [%1.3f,%1.3f,%1.3f]\
                  --boxdim [%d,%d,%d] -o workdir_fam/qdock_output/pocs"%(
            self.receptor.pdbqt_path,
            self.box_center[0],self.box_center[1],self.box_center[2],
            self.dims[0]-1,self.dims[1]-1,self.dims[2]-1))        
        pocs = []
        poc_files = os.listdir("workdir_fam/qdock_output/pocs")
        for poc_file in poc_files:
            if "_fp_" not in poc_file:
                continue
            else:
                pocs.append(prody.parsePDB("workdir_fam/qdock_output/pocs/%s"%(poc_file)))
